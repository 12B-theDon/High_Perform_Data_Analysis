...
bool CheckCuda(const cudaError_t error, const char* const expression) {
  ...
  std::fprintf(stderr, "[score_all CUDA ver2 error] %s failed: %s\n",
...
__global__ void ScoreAllKernelVer2(const unsigned char* const grid,
  ...
  const int offset_x = cx[candidate];
  const int offset_y = cy[candidate];
  int sum = 0;
...
void score_all_CUDA_ver2(const std::vector<unsigned char>& grid, const int w,
  ...
  if (valid_input) {
    ...
    if (cuda_ok) {
      ...
      ScoreAllKernelVer2<<<blocks, kThreadsPerBlock, 0, stream>>>(
          cache.grid, w, h, cache.px, cache.py, cache.cx, cache.cy, n, p,
          cache.score);
      cuda_ok =
          CheckCuda(cudaGetLastError(), "ScoreAllKernelVer2 launch") &&
          cuda_ok;
      cuda_ok =
  ...
  if (call_count <= 5 || call_count % 10 == 0) {
    std::printf("[score_all CUDA ver2 cached baseline-kernel] calls=%d "
                "avg=%.6f ms last=%.6f ms h2d=%.6f ms kernel=%.6f ms "
                "d2h=%.6f ms grid_upload=%d candidates=%d scan_points=%d\n",
