...
bool CheckCuda(const cudaError_t error, const char* const expression) {
  ...
  std::fprintf(stderr, "[score_all CUDA ver5 error] %s failed: %s\n",
...
__device__ int WarpReduceSumVer5(int value) {
...
__global__ void ScoreAllKernelVer5Batched(
  ...
  for (int base = 0; base < point_count; base += kPointTile) {
    ...
    if (threadIdx.x < tile_count) {
      s_px[threadIdx.x] = __ldg(&flat_px[point_offset + base + threadIdx.x]);
      s_py[threadIdx.x] = __ldg(&flat_py[point_offset + base + threadIdx.x]);
    }
  ...
  if (valid_candidate) {
    const int warp_sum = WarpReduceSumVer5(sum);
    if (lane == 0) {
...
void score_all_CUDA_ver5_batched(
  ...
  const int scan_count = std::min(point_offsets.size(), point_counts.size());
  const int cand_scan_count = std::min(cand_offsets.size(), cand_counts.size());
  const int total_points = std::min(flat_px.size(), flat_py.size());
  score->assign(n, 0.0f);
  ...
  if (valid_input) {
    ...
    if (cuda_ok) {
      ...
      ScoreAllKernelVer5Batched<<<blocks, kThreadsPerBlock, 0, stream>>>(
          cache.grid, w, h, cache.flat_px, cache.flat_py, cache.point_offsets,
          cache.point_counts, cache.cand_offsets, cache.cand_counts, cache.cx,
          cache.cy, scan_count, cache.score);
      cuda_ok =
          CheckCuda(cudaGetLastError(), "ScoreAllKernelVer5Batched launch") &&
          cuda_ok;
      cuda_ok =
  ...
  if (call_count <= 10 || call_count % 100 == 0) {
    ...
    std::printf("[score_all CUDA ver5 batched profile] calls=%d avg=%.6f ms "
                "last=%.6f ms h2d=%.6f ms kernel=%.6f ms d2h=%.6f ms "
                "host_overhead=%.6f ms scans=%d candidates=%d "
                "scan_points=%d max_candidates_per_scan=%d blocks_x=%d "
                "total_blocks=%d threads_per_block=%d grid_upload=%d "
                "grid_mib=%.6f pxpy_mib=%.6f scan_meta_mib=%.6f "
                "cxcy_mib=%.6f score_mib=%.6f\n",
                call_count, total_ms / call_count, ms, h2d_ms, kernel_ms,
                d2h_ms, host_overhead_ms, scan_count, n, total_points,
                max_candidates_per_scan, blocks_x, total_blocks,
                kThreadsPerBlock, grid_uploaded ? 1 : 0, grid_mib, pxpy_mib,
                scan_meta_mib, cxcy_mib, score_mib);
    std::fflush(stdout);
  }
}
