...
#ifdef CARTOGRAPHER_PARALLEL_USE_CPU_PARALLEL_SCORE
...
#ifndef CARTOGRAPHER_PARALLEL_MATCHER_PROFILE_LABEL
#define CARTOGRAPHER_PARALLEL_MATCHER_PROFILE_LABEL "baseline"
#endif

#ifdef CARTOGRAPHER_PARALLEL_USE_CPU_PARALLEL_SCORE
#define CARTOGRAPHER_PARALLEL_SCORE_ALL score_all_cpu_parallel
#else
#define CARTOGRAPHER_PARALLEL_SCORE_ALL score_all
#endif
...
void FastMatcher::Score(const Grid& grid, const std::vector<Scan>& scans,
  ...
  for (size_t s = 0; s < scans.size(); ++s) {
    ...
    CARTOGRAPHER_PARALLEL_SCORE_ALL(grid.cell, grid.w, grid.h, scans[s].x,
  ...
  *out = MatchOut();
  ...
  if (coarse.empty()) {
    ...
    if (empty_call_count <= 5 || empty_call_count % 10 == 0) {
      std::printf("[MatchWithWindow %s empty] calls=%d "
                  ...
                  CARTOGRAPHER_PARALLEL_MATCHER_PROFILE_LABEL,
                  empty_call_count,
                  MsBetween(t_total0, t_total1),
                  MsBetween(t_scans0, t_scans1),
                  MsBetween(t_bounds0, t_bounds1),
                  MsBetween(t_grid0, t_grid1),
                  MsBetween(t_lowcand0, t_lowcand1),
                  MsBetween(t_score0, t_score1),
                  scans.size());
      std::fflush(stdout);
    }
  ...
  if (match_call_count <= 5 || match_call_count % 10 == 0) {
    std::printf("[MatchWithWindow %s] calls=%d "
                ...
                "scans=%zu coarse=%zu ok=%d best_score=%.6f\n",
                CARTOGRAPHER_PARALLEL_MATCHER_PROFILE_LABEL,
                match_call_count,
