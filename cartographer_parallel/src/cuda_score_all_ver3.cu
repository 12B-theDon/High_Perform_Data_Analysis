...
bool CheckCuda(const cudaError_t error, const char* const expression) {
  ...
  std::fprintf(stderr, "[score_all CUDA ver3 error] %s failed: %s\n",
...
__global__ void ScoreAllKernelVer3(const unsigned char* const grid,
                                   const int w, const int h,
                                   const int* const px,
                                   const int* const py, const int min_x,
                                   const int min_y, const int step,
                                   const int num_y, const int n,
                                   const int p, float* const score) {
  const int candidate = blockIdx.x * blockDim.x + threadIdx.x;
  if (candidate >= n) return;

  const int ix = candidate / num_y;
  const int iy = candidate - ix * num_y;
  const int offset_x = min_x + ix * step;
  const int offset_y = min_y + iy * step;
  int sum = 0;

  for (int point = 0; point < p; ++point) {
    const int x = px[point] + offset_x;
    const int y = py[point] + offset_y;
    if (x >= 0 && x < w && y >= 0 && y < h) {
  ...
  score[candidate] =
      static_cast<float>(sum) / (255.0f * static_cast<float>(p));
}

}  // namespace

void score_all_CUDA_ver3_bounds(const std::vector<unsigned char>& grid,
                                const int w, const int h,
                                const std::vector<int>& px,
                                const std::vector<int>& py, const int min_x,
                                const int max_x, const int min_y,
  ...
  const int num_x =
      step > 0 && min_x <= max_x ? (max_x - min_x) / step + 1 : 0;
  const int num_y =
      step > 0 && min_y <= max_y ? (max_y - min_y) / step + 1 : 0;
  const int n = num_x > 0 && num_y > 0 ? num_x * num_y : 0;
  score->assign(n, 0.0f);
  ...
  if (valid_input) {
    ...
    if (cuda_ok) {
      ...
      ScoreAllKernelVer3<<<blocks, kThreadsPerBlock, 0, stream>>>(
          cache.grid, w, h, cache.px, cache.py, min_x, min_y, step, num_y, n,
          p, cache.score);
      cuda_ok =
          CheckCuda(cudaGetLastError(), "ScoreAllKernelVer3 launch") &&
          cuda_ok;
      cuda_ok =
  ...
  if (call_count <= 5 || call_count % 10 == 0) {
    std::printf("[score_all CUDA ver3 bounds-indexed] calls=%d avg=%.6f ms "
