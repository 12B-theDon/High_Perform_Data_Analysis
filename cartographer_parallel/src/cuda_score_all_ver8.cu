...
bool CheckCuda(const cudaError_t error, const char* const expression) {
  ...
  std::fprintf(stderr, "[score_all CUDA ver8 error] %s failed: %s\n",
...
__global__ void ScoreAllKernelVer8Batched(
  ...
  int sum = 0;
  if (full_inside) {
    const int candidate_cell = offset_y * w + offset_x;
    for (int i = 0; i < point_count; ++i) {
      const int point_index = point_offset + i;
      sum += __ldg(&grid[candidate_cell + __ldg(&flat_cell[point_index])]);
    }
  } else {
    for (int i = 0; i < point_count; ++i) {
      const int point_index = point_offset + i;
...
void score_all_CUDA_ver8_batched(
  ...
  const int bounds_scan_count = static_cast<int>(
      std::min({scan_min_x.size(), scan_max_x.size(), scan_min_y.size(),
                scan_max_y.size()}));
  const int total_points = static_cast<int>(
      std::min({flat_px.size(), flat_py.size(), flat_cell.size()}));
  score->assign(n, 0.0f);
  ...
  if (valid_input) {
    ...
    if (cuda_ok) {
      for (int chunk_begin = 0; chunk_begin < max_candidates_per_scan;
        ...
        ScoreAllKernelVer8Batched<<<blocks, kThreadsPerBlock, 0, stream>>>(
            cache.grid, w, h, cache.flat_px, cache.flat_py,
            cache.flat_cell, cache.point_offsets, cache.point_counts, cache.cand_offsets,
        ...
        ++chunks;
        cuda_ok =
            CheckCuda(cudaGetLastError(), "ScoreAllKernelVer8Batched launch") &&
            cuda_ok;
        if (!cuda_ok) break;
      }
      cuda_ok =
  ...
  if (call_count <= 10 || call_count % 100 == 0) {
    ...
    std::printf("[score_all CUDA ver8 batched profile] calls=%d avg=%.6f ms "
                ...
                "grid_upload=%d sorted_offsets=1 duplicate_compaction=0 "
                "scan_compression=0 grid_mib=%.6f point_mib=%.6f "
                ...
                grid_mib, point_mib, scan_meta_mib, cxcy_mib, score_mib);
    std::fflush(stdout);
  }
}
